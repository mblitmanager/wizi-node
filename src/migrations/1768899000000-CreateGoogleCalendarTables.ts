...
export class CreateGoogleCalendarTables1768899000000 implements MigrationInterface {
  name = "CreateGoogleCalendarTables1768899000000";

  public async up(queryRunner: QueryRunner): Promise<void> {
    // Create google_calendars table if it doesn't exist
    ...
    // Create google_calendar_events table if it doesn't exist
    await queryRunner.query(`
    ...
    // Add foreign keys if they don't exist (can be tricky with IF NOT EXISTS, so try-catch might be better or just let it be)
    try {
      await queryRunner.query(`
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`google_calendar_events\` DROP FOREIGN KEY \`FK_google_calendar_events_google_calendar_id\``
    );
    await queryRunner.query(
      `ALTER TABLE \`google_calendars\` DROP FOREIGN KEY \`FK_google_calendars_user_id\``
    );
