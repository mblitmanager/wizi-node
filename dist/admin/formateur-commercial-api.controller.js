...
return this.apiResponse.success(data);
arenaRanking(, period, string = "all", , formationId ?  : number);
{
...
sendNotification(, req, any, , data, any);
{
...
getFormationsPerformance(, req);
{
    const data = await this.adminService.getFormateurFormations(req.user.id);
    return this.apiResponse.success(data);
}
getStagiaireFormations(, id, number);
{
    const data = await this.adminService.getStagiaireFormationPerformance(id);
    return this.apiResponse.success(data);
}
getQuizSuccessRate(, period, number = 30, , req);
{
...
getCompletionTime(, period, number = 30, , req);
{
    const formateur = await this.formateurRepository.findOne({
        where: { user_id: req.user.id },
        relations: ["stagiaires"],
...
getActivityHeatmap(, period, number = 30, , req);
{
    const formateur = await this.formateurRepository.findOne({
        where: { user_id: req.user.id },
        relations: ["stagiaires"],
...
getDashboard(, period, number = 30, , req);
{
    const formateur = await this.formateurRepository.findOne({
        where: { user_id: req.user.id },
        relations: ["stagiaires"],
